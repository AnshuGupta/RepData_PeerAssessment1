...
```{r}
daysteps <- tapply(DF$steps, DF$date, function(x) {sum(x, na.rm = TRUE)})
```
...
```{r}
intsteps <- tapply(DF$steps, DF$interval, function(x) {mean(x, na.rm = TRUE)})
```
...
```{r}
totNA <- apply(DF, 2, function(x) {sum(is.na(x))})
totNA
```
...
```{r fig.width=7, fig.height=6}
newdaysteps <- tapply(newDF$steps, newDF$date, function(x) {sum(x)})
...
newDF$weekend <- (weekdays(newDF$date) == "Saturday" | weekdays(newDF$date) == "Sunday")
